...
//    }
//
//    function allTokensOf(address owner) internal view returns (uint[] memory) {
...
//    }
//
//    function _getTokenURI(uint256 tokenId) virtual view internal returns (string memory);
//    function _clearTokenURI(uint256 tokenId) virtual internal;
//
//    /**
...
//    mapping (uint64 => uint256) public shortId2TokenId;
//    mapping (uint256 => uint64) public tokenId2ShortId;
//
//    constructor(string memory name, string memory symbol, string memory baseURI) ERC721(name, symbol) public {
//        _setBaseURI(baseURI);
//    }
//
//    function allTokenInfoOf(address owner) external view returns (uint[] memory, uint64[] memory) {
//        uint[] memory tokenIdList = allTokensOf(owner);
//        uint64[] memory shortIdList = new uint64[](tokenIdList.length);
//        for(uint i = 0; i < tokenIdList.length; i++) {
//            uint tokenId = tokenIdList[i];
//            shortIdList[i] = tokenId2ShortId[tokenId];
//        }
//        return (tokenIdList, shortIdList);
//    }
//
//    // remove the bidirectional mapping
//    function _clearTokenURI(uint256 tokenId) internal override {
//        uint64 shortId = tokenId2ShortId[tokenId];
//        if(shortId != 0) {
//            shortId2TokenId[shortId] = 0;
//            tokenId2ShortId[tokenId] = 0;
//        }
//    }
//
//    // get a string for URI using the shortId
//    function _getTokenURI(uint256 tokenId) internal view override returns (string memory) {
//        uint64 id = tokenId2ShortId[tokenId];
//        return ShortId.shortIdToURI(id);
//    }
//
//    // try to find an unused shortId for tokenId
//    function _registerShortId(uint tokenId) private {
//        uint tmp = tokenId;
//        while(true) {
//            for(uint i = 0; i < 5; i++) {
//                uint64 id = ShortId.toValidShortId(tmp >> (i*48));
//                if (shortId2TokenId[id] == 0) {
//                    shortId2TokenId[id] = tmp;
//                    tokenId2ShortId[tmp] = id;
//                    return;
//                }
//            }
//            tmp = uint256(keccak256(abi.encodePacked(tmp)));
//        }
//    }
//
